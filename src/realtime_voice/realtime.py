"""One speak-and-listen turn over the OpenAI Realtime API (GA interface).
...
async def _receiver(ws, audio: AudioIO, st: _State) -> None:
    try:
        async for raw in ws:
            ev = json.loads(raw)
            etype = ev.get("type", "")
            if etype in ("response.output_audio.delta", "response.audio.delta"):
                audio.enqueue_playback(base64.b64decode(ev["delta"]))
            elif etype in ("session.updated",):
                st.session_ready.set()
...
async def _mic_sender(ws, audio: AudioIO) -> None:
    try:
        while True:
            frame = await audio.mic_frames.get()
...
async def run_turn(
    ...
    *,
    ...
    async with websockets.connect(
        ...
        recv_task = asyncio.create_task(_receiver(ws, audio, st))
        send_task = asyncio.create_task(_mic_sender(ws, audio))
        barged = False
        try:
            ...
            if message:
                ...
                audio.set_mic(bool(listen and barge_in))
                await ws.send(json.dumps({
                ...
                barged = await _speak_phase(ws, audio, st)
...
async def _speak_phase(ws, audio: AudioIO, st: _State) -> bool:
